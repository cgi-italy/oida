    ...
    return (
        <div className='dataset-video-controls'>
            <Tooltip title='Go to video beginning'>
                <Button disabled={isPlaying} onClick={() => props.datasetViz.source.seek(0)}>
                    <FastBackwardOutlined />
                </Button>
            </Tooltip>
            <Tooltip title='Step backward'>
                <Button disabled={isPlaying} onClick={() => props.datasetViz.source.stepBackward()}>
                    <StepBackwardOutlined />
                </Button>
            </Tooltip>
            {!isPlaying && (
            ...
            <Tooltip title='Step forward'>
                <Button disabled={isPlaying} onClick={() => props.datasetViz.source.stepForward()}>
                    <StepForwardOutlined />
                </Button>
            </Tooltip>
            <Tooltip title='Go to video end'>
                <Button disabled={isPlaying} onClick={() => props.datasetViz.source.seek(props.datasetViz.source.duration)}>
                    <FastForwardOutlined />
                </Button>
            </Tooltip>
        </div>
