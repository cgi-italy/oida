...
export const DatasetRasterSequencePercentilesSlider = (props: DatasetRasterSequencePercentilesSliderProps) => {
    ...
    const referenceItem = props.data.length ? props.data[0].stats?.percentiles : undefined;
    if (!referenceItem) {
        return null;
    }

    const min = Math.floor(props.data.reduce((min, item) => {
        const percentiles = item.stats?.percentiles;
        if (percentiles) {
    ...
    const chartSeries = referenceItem.map((percentile) => {
        ...
        const series = {
            ...
            lineColor: chroma(props.color!).darken(2 * (1 - medianDistance)).hex(),
