...
export class TimeIntervalSet {
    ...
    addInterval(interval: TimeInterval): TimeInterval[] {
        let insertionData = this.getOverlappingData(interval);
        ...
        if (insertionData.overlapStart === -1) {
            ...
            let mergeInterval = this.intervals_[insertionData.overlapStart];
            ...
            let endInterval = this.intervals_[insertionData.overlapEnd];
    ...
    removeInterval(interval) {
        let overlapData = this.getOverlappingData(interval);
        if (overlapData.overlapStart !== -1) {
            let firstInterval = this.intervals_[overlapData.overlapStart];
            ...
            let lastInterval = this.intervals_[overlapData.overlapEnd];
            if (lastInterval.end.isAfter(interval.end)) {
                lastInterval.setStart(interval.end);
                deleteEnd--;
            }

            let deleteRange = deleteEnd - deleteStart;
            if (deleteRange > 0) {
    ...
    private getOverlappingData(interval: TimeInterval) {
        ...
        let prevIdx = this.intervals_.length - 1, overlapStart = -1, overlapEnd = -1;
        for (let idx = 0; idx < this.intervals_.length; ++idx) {
            let item = this.intervals_[idx];
